...
app.get("/api/pterodactyl/createadmin", async (req, res) => {
    let { domain, ptla, username } = req.query; // Hapus ptlc, loc, eggid, nestid, ram, disk, cpu karena tidak relevan untuk user saja
    if (!domain || !ptla || !username) { // Sesuaikan parameter yang wajib
        return res.json("Isi Parameternya: domain, ptla, username!"); // Pesan error disesuaikan
    }
    try {
        let apikey = ptla;
        domain = "https://" + domain;
        ...
        let name = capital(username); // Nama user admin, bisa disesuaikan
        ...
        let f = await fetch(domain + "/api/application/users", {
            ...
            "body": JSON.stringify({
                ...
                "root_admin": true, // Ini yang membuat user sebagai admin
                "language": "en",
        ...
        if (data.errors) {
            console.error("Error creating user:", data.errors); // Log error untuk debugging
            return res.json({
                status: false,
                creator: global.creator,
        ...
        // Hapus semua kode di bawah ini yang berkaitan dengan pembuatan server:
        // - Fetch untuk nests/eggs
        // - Fetch untuk servers (POST request)
        // - Variabel desc, usr_id, f1, data2, startup_cmd, f2, result, server

        // Kembalikan respons yang relevan untuk pembuatan user admin
        return res.json({
            status: true,
            creator: global.creator,
            result: {
                ...
                email: user.email,
                password: password, // Penting: Pertimbangkan keamanan pengiriman password langsung
        ...
        console.log(error);
        res.status(500).json({ error: error.message || "An error occurred during user creation." });
    }
});
