...
app.get("/api/pterodactyl/createadmin", async (req, res) => {
    ...
    try {
        let apikey = ptla;
        let capiley = ptlc; // Variabel ini tidak digunakan di sini, tetapi ada.
        let disknya = disk;
        domain = "https://" + domain;
        ...
        let f = await fetch(domain + "/api/application/users", {
            ...
            "body": JSON.stringify({
                ...
                "first_name": name,
                "last_name": "Server",
                "root_admin": true, // Ini akan membuat user admin
                "language": "en",
    ...
    } catch (error) { // <-- Penutup try dan catch
        console.log(error);
        res.status(500).send(error.message || "An error occurred during server creation."); // Lebih baik kirim error message
    }
}); // <-- Penutup route handler /api/pterodactyl/createadmin
